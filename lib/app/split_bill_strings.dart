...
class SplitStrings {
  ...
  String participantCount(int count) =>
      isEnglish ? '$count people' : '$count orang';
  String get totalSaved => isEnglish ? 'Saved total' : 'Total tersimpan';
  String get localOnly =>
      isEnglish ? 'Saved on this device.' : 'Tersimpan di perangkat ini.';
  ...
  String get peopleIntro =>
      isEnglish ? 'Add at least two people.' : 'Tambahkan minimal dua peserta.';
  ...
  String get noPeopleMessage =>
      isEnglish ? 'Add names one by one.' : 'Masukkan nama satu per satu.';
  String get items => isEnglish ? 'Items' : 'Item';
  String get itemsIntro => isEnglish
      ? 'Add items and assign people.'
      : 'Tambah item dan pilih peserta.';
  ...
  String get noItemsMessage => isEnglish
      ? 'Add an item, then assign people.'
      : 'Tambah item, lalu pilih peserta.';
  ...
  String get chargesIntro => isEnglish
      ? 'Add tax, service, or discount.'
      : 'Isi pajak, layanan, atau diskon.';
  String get customIntro =>
      isEnglish ? 'Set each person amount.' : 'Isi jumlah tiap peserta.';
  String get billTotal => isEnglish ? 'Bill total' : 'Total tagihan';
  String get totalToSplit => isEnglish ? 'Total to split' : 'Total dibagi';
  String get allocationOk =>
      isEnglish ? 'Allocation is complete.' : 'Alokasi sudah pas.';
  String remainingAmount(String amount) =>
      isEnglish ? 'Remaining $amount.' : 'Sisa $amount.';
  String get subtotal => 'Subtotal';
  String get subtotalFromItems =>
      isEnglish ? 'Subtotal from items' : 'Subtotal dari item';
  ...
  String get discardMessage => isEnglish
      ? 'Unsaved data will be removed.'
      : 'Data belum tersimpan akan hilang.';
  String get discard => isEnglish ? 'Discard' : 'Buang';
  String removeParticipantTitle(String name) =>
      isEnglish ? 'Remove $name?' : 'Hapus $name?';
  String get removeParticipantMessage => isEnglish
      ? 'Assignments will be cleared.'
      : 'Assignment akan dibersihkan.';
  String get helperNeedPeople =>
      isEnglish ? 'Add at least two people.' : 'Tambahkan minimal dua peserta.';
  String get helperNeedItems =>
      isEnglish ? 'Add at least one item.' : 'Tambahkan minimal satu item.';
  String get helperNeedTotal =>
      isEnglish ? 'Enter the bill total.' : 'Masukkan total tagihan.';
  String get helperNeedCustom =>
      isEnglish ? 'Set remaining to zero.' : 'Selesaikan sisa ke Rp0.';
  String get result => isEnglish ? 'Result' : 'Hasil';
  String get splitBillSummary =>
  ...
  String totalParticipants(int count) =>
      isEnglish ? 'People: $count' : 'Peserta: $count';
  String get roundingNote => isEnglish
      ? 'Rounding keeps the total exact.'
      : 'Pembulatan menjaga total tetap pas.';
  String get billNotFound =>
      isEnglish ? 'Bill not found' : 'Bill tidak ditemukan';
  String get billNotFoundMessage =>
      isEnglish ? 'This bill is unavailable.' : 'Bill ini tidak tersedia.';
  ...
  String modeDescription(SplitMode mode) {
    return switch (mode) {
      SplitMode.equal =>
        isEnglish ? 'Split the total evenly.' : 'Bagi total sama rata.',
      SplitMode.items =>
        isEnglish ? 'Assign items to people.' : 'Tetapkan item ke peserta.',
      SplitMode.custom =>
        isEnglish ? 'Set each amount manually.' : 'Isi jumlah tiap peserta.',
    };
  }
  ...
  String validationMessage(String message) {
    ...
    if (message == 'Could not load saved bills.') {
      return 'Riwayat gagal dimuat.';
    }
    if (message == 'Create a bill first.') return 'Buat bill dulu.';
    if (message == 'Add at least one participant.') {
    ...
    if (message == 'Custom amount cannot be negative.') {
      return 'Jumlah custom tidak boleh negatif.';
    }
    if (message == 'Custom split remaining must be zero.') {
      return helperNeedCustom;
    }
    if (message == 'Bill total invariant failed.') {
      return 'Total bill tidak valid.';
    }
    if (message.startsWith('Enter price for ')) {
      final name = message
          .substring('Enter price for '.length)
          .replaceAll('.', '');
      return 'Isi harga untuk $name.';
    }
    if (message.startsWith('Assign ') &&
        message.endsWith(' to at least one person.')) {
      final name = message.substring(
        'Assign '.length,
        message.length - ' to at least one person.'.length,
      );
